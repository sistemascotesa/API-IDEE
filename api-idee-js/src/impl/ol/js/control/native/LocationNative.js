...
import ControlNative from './ControlNative';
 ...
 *  @api
 */
class LocationNative extends ControlNative {
  /**
   ...
   * @param {HTMLElement} element Contenedor HTML, con la vista en este caso un boton
  ...
  constructor(tracking, highAccuracy, maximumAge, facadeMap, element, vendorOptions) {
    super({
      facadeMap,
      element,
      vendorOptions,
    ...
    olPositionFeature.setStyle(LocationNative.POSITION_STYLE);
    ...
    if (isNullOrEmpty(this.geolocation_)) {
      ...
      this.geolocation_.on('change:position', (evt) => {
        ...
        if (this.element.classList.contains('m-locating')) {
          this.facadeMap_.setZoom(LocationNative.ZOOM); // solo 1a vez
        }
 ...
 */
LocationNative.POSITION_STYLE = new OLStyle({
 ...
 * @api stable
 */
Object.defineProperty(LocationNative, 'ZOOM', {
  get() {
    return IDEE.config.ZOOM_LOCATION;
  },
});

export default LocationNative;
